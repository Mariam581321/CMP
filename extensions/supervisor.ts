...
//
// One pi process per attempt, one session file, one continuous event stream, and
// per-extension state (lean_check's unchanged-file note) survives the whole attempt.
// pi explicitly supports this: messages queued by agent_end extension handlers trigger
// a continuation inside the same headless prompt() await (agent-session.ts,
// _handlePostAgentRun).
//
...
export default function (pi: ExtensionAPI) {
  ...
  // counter is then the ONLY bound on an error loop short of the 48 h wall backstop.
  // run.js does not pass this knob — the constant IS the policy; cfg only exists so
  // scripts/probe-supervisor.mjs can drive the give-up branch.
  ...
  // Worker spend: the soft stop must count it, or it would keep nudging into
  ...
  async function decide(event?: any) {
    ...
    // like "no progress" forever, ending attempts after a few consecutive errors with
    // the budget untouched. It gets its own, much longer ledger instead — and the SAME
    // nudge below, because once pi's retries ARE spent the
    ...
    // keeps the loop hot, spending on a retry storm while nothing can be checked.
    // Waiting costs zero tokens, and the STOP file still aborts cleanly.
    //
    ...
    for (;;) {
      try {
        check = origPath && existsSync(origPath)
          ...
          // turn resends, so a full digest per nudge would put hundreds of KB of
          // duplicated compiler output permanently in the window. renderCheck protects
          // the header — every sorry line, the statement and axiom verdicts — at any cap.
    ...
    // with the statement intact, and without that the supervisor would bless as done a
    // file the grader fails as bad_axioms.
    ...
    // Same paragraphs lean_check uses for the same faults (blockerNotes),
