...
// 7-9. The heartbeat note: exactly one per check, only when something timed out, and the
// same one whichever budget it was — prepare() sets both to the cap and clampHeartbeats
// lets a file lower either, so one sentence is true of every timeout Lean can print.
{
